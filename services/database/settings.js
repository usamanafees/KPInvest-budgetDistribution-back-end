...
const getBonusPoolById = async (Id) => {
    return await Settings.findByPk(Id);
} 

const updateBonusPool = async (Id, data) => {
    const updatedBonus =  await Settings.update(data, { returning: true, plain: true, where: {id: Id} });
    // return updatedBonus
    const bonusPool = updatedBonus[1].org_bonus_pool;
    ...
    console.log("total indexes",unique);
    ...
    for (let i = 0; i < unique.length; i++) {
        const el = unique[i];
        const obj = { own: el, bonus_value: 0,
             prev_total: 0, total_aloc_val: 0,
            total_rev_value:0 }
            let to_array= [];
            let to_obj= {};
    ...
    while(chek_flag == 1){
        ...
        for(let z=0;z<flow.length;z++)
        {
            ...
            // console.log("aaaaaaaaaaaaaaa",flow[z].prev_total,"@@@@@@@", flow[z].total_aloc_val);
            // console.log("aaaaaaaaaaaaaaa",Math.round(flow[z].prev_total),"@@@@@@@", Math.round(flow[z].total_aloc_val));
            // console.log("======-=====-==================");
    ...
    console.log("last flowwwwwwwwwwwww  ggg total iterations",count_init);
    return updatedBonus;
}
